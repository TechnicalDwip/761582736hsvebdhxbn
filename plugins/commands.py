...
async def start(client, message):
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [[
                    ...
                    InlineKeyboardButton('ᴏᴡɴᴇʀ 😼', user_id=int(1782834874)),
    ...
    if AUTH_CHANNEL and not await is_subscribed(client, message):
        ...
        btn = [
            [
                InlineKeyboardButton("Jᴏɪɴ Cʜᴀɴɴᴇʟ Fɪʀsᴛ", url=invite_link.invite_link)
            ],[
                InlineKeyboardButton('Cʜᴇᴋ Aɢᴀɪɴ 🔁', callback_data='sinfo')
            ]
        ]
        ...
        if message.command[1] != "subscribe":
            try:
                ...
                btn.append([InlineKeyboardButton("Cʜᴇᴋ Aɢᴀɪɴ 🔁", callback_data=f"checksub#{kk}#{file_id}")])
            except (IndexError, ValueError):
                btn.append([InlineKeyboardButton("Cʜᴇᴋ Aɢᴀɪɴ 🔁", url=f"https://t.me/{temp.U_NAME}?start={message.command[1]}")])
        await client.send_photo(
            chat_id=message.from_user.id,
            photo="https://telegra.ph/file/20b4aaaddb8aba646e53c.jpg",
            caption="**If you want the movies file, Join our back-up channel, then click on the 'Chek Again' button below...\n\nThen you will get the movie files...**",
    ...
    if data.startswith("sendfiles"):
        ...
        if temp.SHORT.get(user)==None:
            await message.reply_text(text="<b>Please Search Again in [Group](t.me/SwiftHornRequest)</b>")
        else:
            chat_id = temp.SHORT.get(user)
    ...
    if not files_:
        ...
        try:
            ...
            title = '@SwiftHornCinema' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), file.file_name.split()))
            ...
            k = await msg.reply("<b>Above File Will be Delete in 10 min (Due To Copyright Issues)</b>",quote=True)
            await asyncio.sleep(600)
            await msg.delete()
    ...
    if CUSTOM_FILE_CAPTION:
        try:
            ...
            f_caption=f_caption
    if f_caption is None:
        f_caption = f"@SwiftHornCinema  {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))}"
    if not await check_verification(client, message.from_user.id) and VERIFY == True:
        btn = [[
            InlineKeyboardButton("Verify", url=await get_token(client, message.from_user.id, f"https://telegram.me/{temp.U_NAME}?start="))
        ]]
    ...
    k = await msg.reply("<b>Above Files Will be Deleted in 10min (Due To Copyright issues)</b>",quote=True)
    await asyncio.sleep(600)
    await msg.delete()
...
@Client.on_message(filters.command("deletefiles" , "remove") & filters.user(ADMINS))
...
async def settutorial(bot, message):
    ...
    chat_type = message.chat.type
    if chat_type == enums.ChatType.PRIVATE:
        return await message.reply_text("**This Command Work Only in group Try it in your own group**")
    elif chat_type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        grpid = message.chat.id
        title = message.chat.title
    else:
...
async def stop_button(bot, message):
    msg = await bot.send_message(text="**Starting Restart ⏳**", chat_id=message.chat.id)       
    await asyncio.sleep(3)
    await msg.edit("**Bot Restart Successfully ✅**")
